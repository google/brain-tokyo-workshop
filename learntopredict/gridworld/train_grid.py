...
import sys
import config
from model_grid_fc import make_model, simulate #CHANGE TO model_grid_near TO RUN CONV EXPERIMENTS
...
def initialize_settings(sigma_init=0.1, sigma_decay=0.9999):
  ...
  population = num_worker * num_worker_trial
  filebase = 'log/p.' + str(peak_prob) + '.' + gamename+'.'+optimizer+'.'+str(num_episode)+'.'+str(population)
  game = config.games[gamename]
  model = make_model(game, peak=peak_prob)
...
def master():
  ...
  #uniq = str(np.random.rand())
  uniq = str(time.time())

  filename = filebase+'.'+uniq+'.json'
  filename_log = filebase+'.'+uniq+'.log.json'
  filename_hist = filebase+'.'+uniq+'.hist.json'
  filename_best = filebase+'.'+uniq+'.best.json'
  ...
  while t < 4000:
    ...
    sprint(gamename, h, 'weight norm of model: ', np.linalg.norm(np.array(model_params), ord=2))
    ...
    if (t % eval_steps == 0): # evaluate on actual task at hand
      ...
      #model_params_quantized = np.array(es.result()[0]).round(4)
      ...
      with open('best.' + str(peak_prob) + '.' + str(uniq) + '.dat', 'wt') as f:
        f.write(str(peak_prob) + "\t" + str(best_reward_eval))


def main(args):
  global gamename, optimizer, num_episode, eval_steps, num_worker, num_worker_trial, antithetic, seed_start, retrain_mode, cap_time_mode, peak_prob
...
def mpi_fork(n):
  ...
  if os.getenv("IN_MPI") is None:
    ...
    subprocess.check_call(["mpirun", "-np", str(n), sys.executable] +['-u']+ sys.argv, env=env, timeout=2100) #4100 for near
    return "parent"
  else:
...
if __name__ == "__main__":
  ...
  parser.add_argument('--peak', type=float, default=1.0, help='probability of peaking at real environment')
