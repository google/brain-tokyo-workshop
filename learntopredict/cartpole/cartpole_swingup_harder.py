...
class CartPoleSwingUpHarderEnv(gym.Env):
    ...
    def _reset(self):
        ...
        obs = np.array([x, x_dot, np.cos(theta),np.sin(theta), theta_dot]) # set zero for init differences
        return obs

    def _render(self, mode='human', close=False, override_state=None):
        if close:
        ...
        if self.viewer is None:
            ...
            # dream cart
            l,r,t,b = -cartwidth/2, cartwidth/2, cartheight/2, -cartheight/2
            dream_cart = rendering.PolyLine([(l,b), (l,t), (r,t), (r,b)], True)
            ...
            l,r,t,b = -polewidth/2,polewidth/2,polelen-polewidth/2,-polewidth/2
            dream_pole = rendering.PolyLine([(l,b), (l,t), (r,t), (r,b)], True)
            ...
            # Make another circle on the top of the pole
            self.dream_pole_bob = rendering.make_circle(polewidth/2, filled=False)
            ...
            self.dream_wheel_l = rendering.make_circle(cartheight/4, filled=False)
            self.dream_wheel_r = rendering.make_circle(cartheight/4, filled=False)
            self.dream_wheeltrans_l = rendering.Transform(translation=(-cartwidth/2, -cartheight/2))
            self.dream_wheeltrans_r = rendering.Transform(translation=(cartwidth/2, -cartheight/2))
        ...
        self.dream_pole_bob_trans.set_translation(-self.l*np.sin(dream_x[2]), self.l*np.cos(dream_x[2]))
