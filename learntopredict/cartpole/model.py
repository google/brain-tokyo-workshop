...
class SimpleWorldModel:
  ...
  def __init__(self, obs_size=5, action_size=1, hidden_size=80):
    ...
    if self.hard_reward:
      self.shapes = [ (self.obs_size-1 + self.action_size, self.hidden_size), # input layer, but theta -> cos,sin (so + 1)
      ...
      self.shapes = [ (self.obs_size-1 + self.action_size, self.hidden_size), # input layer
                      #(self.hidden_size, self.hidden_size), # hidden layer 1
                      #(self.hidden_size, self.hidden_size), # hidden layer 2
  ...
  def predict_dynamics(self, obs, action):
    ...
    h = np.concatenate([[x_dot,c,s,theta_dot], np.array(action).flatten()])
...
class CustomModel:
  ...
  def __init__(self, game):
    ...
    self.agent = Agent(layer_1=self.agent_hidden_size, layer_2=0, input_size=self.input_size, output_size=1)
  ...
  def get_action(self, prev_obs, t=0, mean_mode=False):
    ...
    action = [all_action[0]]
    #self.peek_prob = sigmoid(all_action[1])
    ...
    next_x_dot_dot, next_theta_dot_dot = self.world_model.predict_dynamics(obs, action)
...
def simulate(model, train_mode=False, render_mode=True, num_episode=5, seed=-1, max_len=-1):
  ...
  for episode in range(num_episode):
      ...
      if render_mode:
        if model.experimental_mode:
          if model.peek:
            ...
            render_screen = model.env._render(render_flag, override_state=state)
        else:
          render_screen = model.env.render(render_flag)
        if RENDER_DELAY:
          time.sleep(0.01)
        ...
        if record_rgb:
          ...
          imageio.imwrite(os.path.join(video_path,(format(t, "05d")+".png")), img)
    ...
    if render_mode:
      ...
      if model.experimental_mode:
        print("actual reward", actual_reward, "percent glimpse", float(num_glimpse)/float(t+1.0))
