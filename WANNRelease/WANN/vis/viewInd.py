...
def viewPop(pop, taskName, plt_height=5):
    if len(pop) > 10:
        raise ValueError("Population should be small (less than 10 individuals)")
    
    env = games[taskName]
    
    # Draw Graph
    fig = plt.figure(figsize=(plt_height*len(pop), plt_height), dpi=100)
...
def viewInd(ind, taskName, plt_height=5):
    env = games[taskName]
    if isinstance(ind, str):
        ind = np.loadtxt(ind, delimiter=',') 
        wMat = ind[:,:-1]
        ...
        wMat = ind.wMat
        aVec = ind.node[-1,:]
    print('# of Connections in ANN: ', np.sum(wMat[~np.isnan(wMat)] != 0))
    ...
    # Draw Graph
    fig = plt.figure(figsize=(plt_height,plt_height), dpi=100)
...
def ind2graph(wMat, nIn, nOut):
    ...
    assert wMat.shape[0] == wMat.shape[1]
    G = nx.DiGraph()
    G.add_nodes_from([node for node in range(wMat.shape[0])])
...
def labelInOut(pos, env):
    ...
    for i in range(nIn):
        plt.annotate(labelDict[i], xy=(pos[i][0]-0.5, pos[i][1]),\
                     xytext=(pos[i][0]-2.5, pos[i][1]-0.5),\
                     arrowprops=dict(arrowstyle="->",color='k',\
                                     connectionstyle="angle"))

    for i in range(nNode-nOut,nNode):
        plt.annotate(labelDict[i], xy=(pos[i][0]+0.1, pos[i][1]),\
